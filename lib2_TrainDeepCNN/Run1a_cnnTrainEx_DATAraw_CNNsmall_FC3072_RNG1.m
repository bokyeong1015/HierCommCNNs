...
num_fcHiddenNeuron = 3072; % # neurons in the fully-connected hidden layer, e.g., 3072, 2048, 1024, 512
prob_fcDropout = 0.8; % dropout probability in the fully-connected hidden layer, e.g., 0.8, 0.5

flag_cnn = 'sma'; % size of CNN receptive fields, e.g., 'med' for medium, 'lar' for large, 'sma' for small.
...
imdb1 = load(['imdb42_augX10_SFEWtrain_Raw']) ; 
imdb2 = load(['imdb42_augX10_SFEWvalid_Raw']) ; 
...
net.layers{end+1} = struct('type', 'conv', ...
                       'filters', 0.01 * randn(3, 3, 1, 32, 'single'), ...
...
rng(rngNum)
net.layers{end+1} = struct('type', 'conv', ...
                       'filters', 0.01 * randn(3, 3, 32, 32, 'single'), ...
                       'biases', 0.1 * ones(1, 32, 'single'), ...
                       'stride', 1, ...
                       'pad', 0, ...
                       'filtersLearningRate', 1, ...
...
rng(rngNum)
net.layers{end+1} = struct('type', 'conv', ...
                       'filters', 0.01 * randn(3, 3, 32, 64,'single'), ...
