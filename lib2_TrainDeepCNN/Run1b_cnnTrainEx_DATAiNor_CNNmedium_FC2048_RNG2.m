...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rngNum = 2; % rng number for random weight initialization, e.g., 1,2,3
num_fcHiddenNeuron = 2048; % # neurons in the fully-connected hidden layer, e.g., 3072, 2048, 1024, 512
prob_fcDropout = 0.5; % dropout probability in the fully-connected hidden layer, e.g., 0.8, 0.5

flag_cnn = 'med'; % size of CNN receptive fields, e.g., 'med' for medium, 'lar' for large, 'sma' for small.
...
imdb1 = load(['imdb42_augX10_SFEWtrain_iNor']) ; 
imdb2 = load(['imdb42_augX10_SFEWvalid_iNor']) ; 
...
net.layers{end+1} = struct('type', 'conv', ...
                       'filters', 0.01 * randn(5, 5, 1, 32, 'single'), ...
...
rng(rngNum)
net.layers{end+1} = struct('type', 'conv', ...
                       'filters', 0.01 * randn(4, 4, 32, 32, 'single'), ...
                       'biases', 0.1 * ones(1, 32, 'single'), ...
                       'stride', 1, ...
                       'pad', 1, ...
                       'filtersLearningRate', 1, ...
